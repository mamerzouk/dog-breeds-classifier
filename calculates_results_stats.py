...
def calculates_results_stats(results_dic):
    ...
    # Loop over the result_dic entries to fill the number statistics
    for result in results_dic:
        n_dogs_img += results_dic[result][3]
        n_match += results_dic[result][2]
        # Since we use 0/1 values, we use arithmetic operations (*/+ as and/or) instead of if/else
        n_correct_dogs += results_dic[result][3] * results_dic[result][4]
        n_correct_notdogs += (1 - results_dic[result][3]) * (1 - results_dic[result][4])
        n_correct_breed += (results_dic[result][3]*results_dic[result][4]) * results_dic[result][2]

    # The number of non-dogs images is only (the number of images - the number of dogs images)
    n_notdogs_img = n_images - n_dogs_img
    # We use a try/except to avoid ZeroDivisionError in case there is not a single dog image
    try:
        ...
        pct_correct_notdogs = (n_notdogs_img / (n_images - n_dogs_img)) * 100
    except ZeroDivisionError:
        pct_match = pct_correct_dogs = pct_correct_breed = pct_correct_notdogs = 0

    # We fill the new results_stats_dic dictionary and return it
