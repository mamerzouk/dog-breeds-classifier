...
def classify_images(images_dir, results_dic, model):
    ...
    for path in paths:
        ...
        result = classifier(images_dir+"/"+path, model).lower()
        # We add the calassifier result to the result_dic
        results_dic[path].append(result)
        # We add a 1 mataching bit if the label is in of the classifier's result list
        # The calassifier result is a string of names separated by ', ', so we split it into a list
        if results_dic[path][0] in result.split(', '):
            results_dic[path].append(int(1))
        else:
            results_dic[path].append(int(0))
