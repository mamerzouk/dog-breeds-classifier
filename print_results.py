...
def print_results(results_dic, results_stats_dic, model,
    ...
    # Simply printing the different dictionary entries :
    print("\n\n*** Summary Results On The Classification By The Architecture {} ***"
          .format(model))
    ...
    # This code is run only when print_incorrect_dogs is True
    # It loops over result_dic looking for incorrectly classified dogs (Either
    # a dog classified non-dog or a non-dog classified dog)
    if print_incorrect_dogs:
        print("\n\n*** Incorrectly Classified Dogs ***")
        for result in results_dic:
            # We check if it is a misclassification with the match bit (Index 2)
            if results_dic[result][2] == 0:
                print("-> Images     : {} \n   Labeled    : {} \n   Classified : {}"
                      .format(result, results_dic[result][0], results_dic[result][1]))

    # This code is run only when print_incorrect_breed is True
    # It loops over result_dic looking for incorrectly classified breeds
    if print_incorrect_breed:
        print("\n\n*** Incorrectly Classified Breeds ***")
        for result in results_dic:
            # A breed is misclassification is identified when the image is labled
            # and classified as a dog (condition 2) but the breeds doesn't match (condition 1)
            if ((results_dic[result][2] == 0) and
                    (results_dic[result][3]*results_dic[result][4] == 1)):
                print("-> Images     : {} \n   Labeled    : {} \n   Classified : {}"
                      .format(result, results_dic[result][0], results_dic[result][1]))
